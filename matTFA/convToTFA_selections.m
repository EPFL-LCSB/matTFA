function [model, relaxedDGoVarsValues, modelwDGoSlackVars] = convToTFA_selections(model, ReactionDB, rxnNameListNoThermo, flagInfeasibility, rxnNameListNoDGoRelax, minObjSolVal, flagToAddPotentials, flagToAddLnThermoDisp, verboseFlag, printLP, flagMCA_FarEquilibrium, bigM_value, bigM_thermo_value)
...
if ~exist('bigM_value','var') || isempty(bigM_value)
    bigM_value = 1e6;
end
if ~exist('bigM_thermo_value','var') || isempty(bigM_thermo_value)
    bigM_thermo_value = 1e6;
end

GAS_CONSTANT = 1.9858775/1000; % Kcal/(K mol)
% value for the bigM in THermo constraints. This will also be the bound value
bigMtherm = bigM_thermo_value;
...
if any((model.lb < -bigM_value) | (model.ub > bigM_value))

...
if isfield(model,'metSEEDID')
    model.metSEEDID = model.metSEEDID(selMets);
end
if isfield(model,'metCompSymbol')
    model.metCompSymbol = model.metCompSymbol(selMets);
end
if isfield(model,'metDeltaGFstd')
    model.metDeltaGFstd = model.metDeltaGFstd(selMets);
end
if isfield(model,'metDeltaGFerr')
    model.metDeltaGFerr = model.metDeltaGFerr(selMets);
end
if isfield(model,'metMass')
    model.metMass = model.metMass(selMets);
end
if isfield(model,'metDeltaGFtr')
    model.metDeltaGFtr = model.metDeltaGFtr(selMets);
end
if isfield(model,'struct_cues')
    model.struct_cues = model.struct_cues(selMets);
end

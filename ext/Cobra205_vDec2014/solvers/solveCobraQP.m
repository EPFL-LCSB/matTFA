...
%
% parameters    Structure contain+ing optional parameters as fields.
...
%        = 4 if Cholesky said ADDA was not positive definite.
    ...
    case 'gurobi5'
     ...
     % Free academic licenses for the Gurobi solver can be obtained from
        ...
        % edit the varnames in the gurobi model
        if ~exist('QPproblem.varnames','var')
            QPproblem.varnames = QPproblem.rxns;
        end
        ...
        Q =  QPproblem.F;
%         Q = 0.5*(Q + Q'); %ensure symmetry
%         Q = abs(Q);
        QPproblem.Q = sparse(Q);
        QPproblem.modelsense = QPproblem.osense;
        [QPproblem.A,QPproblem.rhs,QPproblem.obj,QPproblem.sense] = deal(sparse(QPproblem.A),QPproblem.b,double(QPproblem.c),QPproblem.csense);

        % Write model
        gurobi_write(QPproblem, 'qp.lp') 

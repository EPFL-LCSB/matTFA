function [model,hasEffect,constrRxnNames,deletedGenes] = thermoDeleteModelGenes(model,geneList,downRegFraction)
...
% Anush Chiappino-Pepe 8/8/17 - added thermo to the default deleteModelGenes function from COBRA
    ...
    if (~isempty(rxnInd))
        ...
        if (any(constrainRxn))
            ...
            if (nargin > 2)
                ...
                model.var_ub(ismember(model.varNames,strcat('R_', constrRxnNames)))=downRegFraction*model.var_ub(ismember(model.varNames,strcat('R_', constrRxnNames)));
                model.var_ub(ismember(model.varNames,strcat('F_', constrRxnNames)))=downRegFraction*model.var_ub(ismember(model.varNames,strcat('F_', constrRxnNames)));
                model.var_lb(ismember(model.varNames,strcat('R_', constrRxnNames)))=downRegFraction*model.var_lb(ismember(model.varNames,strcat('R_', constrRxnNames)));
                model.var_lb(ismember(model.varNames,strcat('F_', constrRxnNames)))=downRegFraction*model.var_lb(ismember(model.varNames,strcat('F_', constrRxnNames)));
                model.var_lb(ismember(model.varNames,strcat('NF_', constrRxnNames)))=downRegFraction*model.var_lb(ismember(model.varNames,strcat('NF_', constrRxnNames)));
                model.var_ub(ismember(model.varNames,strcat('NF_', constrRxnNames)))=downRegFraction*model.var_ub(ismember(model.varNames,strcat('NF_', constrRxnNames)));
            else
%                 Full deletion
                model = changeRxnBounds(model,constrRxnNames,0,'b');
                model.var_ub(ismember(model.varNames,strcat('R_', constrRxnNames)))=0;
                model.var_ub(ismember(model.varNames,strcat('F_', constrRxnNames)))=0;
                model.var_lb(ismember(model.varNames,strcat('R_', constrRxnNames)))=0;
                model.var_lb(ismember(model.varNames,strcat('F_', constrRxnNames)))=0;
                model.var_lb(ismember(model.varNames,strcat('NF_', constrRxnNames)))=0;
                model.var_ub(ismember(model.varNames,strcat('NF_', constrRxnNames)))=0;
            end
            hasEffect = true;
        end
    end
